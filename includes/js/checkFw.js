...
function CheckFW() {
    ...
    if (ps4Regex.test(userAgent)) {
        ...
        // UI-only mode: recognize any detected PS4 firmware without showing
        // the old "Incompatible" state. Exploit selection remains limited
        // to the firmware ranges that have real exploit support below.
        var uiOnlyUnsupported = !isNaN(fwNum) && !(fwNum >= webKitMin && fwNum <= webKitMax);
        if ((fwNum >= webKitMin && fwNum <= webKitMax) || uiOnlyUnsupported) {
            ...
            if (!uiOnlyUnsupported) {
                updateExploitChainVisibility(fwVersion);
                firstTimeExploitChain(fwVersion);
            } else {
                // UI-only mode for firmware outside the tested exploit ranges:
                // never expose an unsupported exploit chain, but also never show
                // the UI's "Incompatible" state.
                ['choosejb-initial', 'chooseExploitChain', 'exploit-main-screen', 'scrollDown', 'advancedPayloads', 'henSelection', 'autoJbContainer', 'bareboneJBOption'].forEach(function(id) {
                    var el = document.getElementById(id);
                    if (el) el.style.display = 'none';
                });
                document.getElementById('exploitContainer').style.display = 'block';
                if (ui.payloadsSection) ui.payloadsSection.style.margin = 'auto';
